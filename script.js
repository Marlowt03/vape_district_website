...
(() => {
  ...
  nav.addEventListener('click', (e) => {
    ...
    const nestedTrigger = e.target.closest('.submenu > a, .submenu > .has-sub, .submenu > button, .submenu > span');
    if (nestedTrigger) {
  ...
  // Normalize: if an anchor has .has-sub but its closest .submenu has no panel, drop the class
  nav.querySelectorAll('.submenu > a.has-sub').forEach(a => {
    const group = a.closest('.submenu');
    const panel = group?.querySelector(':scope > .sub-dropdown, :scope > .dropdown-content, :scope > .submenu');
    if (!panel) a.classList.remove('has-sub');
  });
